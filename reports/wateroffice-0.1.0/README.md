...
Both Linux amd64 images were pulled anonymously using an empty Docker credential configuration; their image IDs match the tested images. [Build and publication workflow](https://github.com/Ninggggy/sgr-webenv/actions/runs/36222256971). The public data archive was downloaded anonymously in full (407,458,840 bytes) and compared byte-for-byte with the validated archive. The tagged source, release page and repository-linked package pages are public. The shared prepare/start/verify/reset commands passed on native Linux using the verified runtime files, and its installed restricted browser passed all 13 isolation checks. The new preview uses loopback port 8084. The prior development application and archive remain available for rollback.

## Completion

Released as [v0.1.3](https://github.com/Ninggggy/sgr-webenv/releases/tag/v0.1.3), containing Wateroffice application/browser `0.1.0`. Data acquisition is intentionally closed at 483 upgraded stations. No remaining failed required check was found within the documented support scope. Other data/feature boundaries remain described in the data guide.
