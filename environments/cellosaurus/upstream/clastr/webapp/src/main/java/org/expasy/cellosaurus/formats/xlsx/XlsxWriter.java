...
public class XlsxWriter implements Writer {
    ...
    public void add(Search search) {
        ...
        int c = 0;
        currentRow++;
        for (List<Integer> rowPosition : search.getRowOrder()) {
            CellLine cellLine = search.getResults().get(rowPosition.get(0));
            Profile profile = cellLine.getProfiles().get(rowPosition.get(1));
            boolean best = rowPosition.get(1) == 0;
            {
