...
public class JsonFormatter {
    ...
    public Search toSearch(String json) {
        ...
        Search search = new Search(cellLines, parameters, description, cellosaurusRelease, runOn, toolVersion, searchSpace);
        if (searchObject.has("rowOrder")) {
            List<List<Integer>> order = new ArrayList<>();
            try {
                for (JsonElement row : searchObject.getAsJsonArray("rowOrder")) {
                    List<Integer> pair = new ArrayList<>();
                    for (JsonElement index : row.getAsJsonArray()) {
                        if (!index.isJsonPrimitive() || !index.getAsJsonPrimitive().isNumber())
                            throw new IllegalArgumentException("rowOrder indices must be integers");
                        pair.add(index.getAsBigDecimal().intValueExact());
                    }
                    order.add(pair);
                }
            } catch (RuntimeException e) {
                throw new IllegalArgumentException("Invalid rowOrder", e);
            }
            search.setRowOrder(order);
        }
        return search;
    }
