...
public class CsvFormatter {
    ...
    public String toCsv(Search search) {
        ...
        sb.append("\"");
        sb.append(FormatsUtils.makeMetadata(search).replace("\"", "\"\""));
        sb.append("\"\r\n");
        ...
        for (List<Integer> position : search.getRowOrder()) {
            CellLine cellLine = search.getResults().get(position.get(0));
            Profile profile = cellLine.getProfiles().get(position.get(1));
            boolean best = position.get(1) == 0;
            {
                sb.append('"');
                ...
                sb.append(',');
                sb.append('"');
                sb.append(cellLine.getName().replace("\"", "\"\""));
                sb.append('"');
                sb.append(',');
                sb.append('"');
