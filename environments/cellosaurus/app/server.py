 ...
 def handle_request(self,post=False):
  ...
  try:
   ...
   if path in ['/coverage','/license','/outside','/api','/description.html','/contact','/sars-cov-2.html','/educational_resources.html','/overview_rii.html','/SAB.html','/references.html']:
    body='<p><strong>Application 0.1.0: core Cellosaurus website and CLASTR.</strong> Search sample counts and complete candidate sets have been verified. Case-insensitive equal-name ties have no fixed original-site order; the offline site uses accession order within ties. Large searches return the full set, including results beyond the original HTML display limit. Unverified syntax is explicitly rejected. STR results retain complete source names where the original truncates text at XML entities. Source-specific STR rows remain visible. Detail links prefill the original-site selected marker values. CLASTR CSV and XLSX follow the displayed profile-row order. JSON retains grouped results and includes rowOrder (result index, profile index) to specify that same display order.</p><p>Complete release 56 source data, with documented original-site compatibility differences. This environment includes core records and CLASTR, not the complete REST API, RDF/SPARQL or external database pages.</p><p>Search uses a local candidate index and explicit query rules. Equivalence is limited to the documented validated behavior; this is not the complete REST API. Unknown syntax is rejected rather than ignored.</p><p>Data: CALIPHO / SIB, Creative Commons Attribution 4.0 (<a href="https://creativecommons.org/licenses/by/4.0/">license terms</a>). CLASTR code: GPL-3.0; <a href="/CLASTR-LICENSE.txt">license</a>. Offline modifications include local data loading and isolated deployment.</p>'
    if q.get('url'):body+='<p>Outside archived scope: '+esc(q['url'][0])+'</p>'
    self.send(200,page('Coverage, attribution and limits',body));return
   if path=='/faq':
