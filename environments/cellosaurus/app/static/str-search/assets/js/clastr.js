let activeQuery = null, activeBatch = null, activeReader = null, queryEpoch = 0;
const activeExports = new Set();
function cancelPending() {
    queryEpoch++;
    if (activeQuery) activeQuery.abort();
    if (activeBatch) activeBatch.abort();
    for (const request of activeExports) request.abort();
    activeExports.clear();
...
function parseURLVariables() {
    if (window.location.search.length > 0) {
        ...
        for (const [parameter, value] of new URLSearchParams(window.location.search)) {
            let key = parameter.split(" ").join("_");
            ...
            if (key === "name") {
                ...
                switchSpecies("mouse");
                if (key !== "Mouse_STR_9-2" && document.getElementById("input-" + key)) document.getElementById("input-" + key).value = value;
            } else if (key.startsWith("Dog")) {
                switchSpecies("dog");
                if (document.getElementById("input-" + key)) document.getElementById("input-" + key).value = value;
            } else {
...
function search() {
    cancelPending();
    const ticket = queryEpoch;
    jsonResponse = undefined;
    $('#table-results').empty();$('#results').hide();
    let jsonQuery = {};
    ...
    activeQuery = $.ajax({
        ...
        error: function (response, status, xhr) {
            if (ticket !== queryEpoch || status === "abort") return;
...
let table = {
    build: function (json) {
        ...
        for (let i = 0; i < json.results.length; i++) {
            for (let a in json.results[i].profiles) {
                ...
                htmlContent += "<tr data-result=\"" + i + "\" data-profile=\"" + a + "\">";
                ...
                htmlContent += "<td>" + escapeHtml(json.results[i].name) + "</td>";
    ...
    sortState: null,
    sort: function (column, ascending, remember = true) {
        const node = document.getElementById('table-results');
        ...
        const rows = Array.from(node.querySelectorAll('tr[data-result]'));
        rows.sort((a, b) => {
            const x = value(a), y = value(b);
            const numeric = column === 2 || column === 3;
            const difference = numeric ? parseFloat(x) - parseFloat(y) : x.localeCompare(y);
            // Ties retain the upstream query/profile order in either direction.
            return (ascending ? difference : -difference) ||
                Number(a.dataset.result) - Number(b.dataset.result) ||
                Number(a.dataset.profile) - Number(b.dataset.profile);
        });
        const parent = rows.length ? rows[0].parentNode : node;
        rows.forEach(row => parent.appendChild(row));
        headers.forEach((th, index) => {
            th.querySelector('.sort-by').className = 'sort-by' + (index === column ? (ascending ? ' asc' : ' des') : '');
            th.setAttribute('aria-sort', index === column ? (ascending ? 'ascending' : 'descending') : 'none');
        });
        table.sortState = {column, ascending};
        if (remember) document.dispatchEvent(new Event('clastr-sort'));
    },
    _makeSortable: function () {
        document.querySelectorAll('#table-results th').forEach((th, column) => {
            th.addEventListener('click', () => table.sort(column,
                table.sortState && table.sortState.column === column ? !table.sortState.ascending : true));
        });
        if (table.sortState) table.sort(table.sortState.column, table.sortState.ascending, false);
    }
...
let importFile = {
    read: function (files) {
        ...
        if (files[0].size > 16 * 1024 * 1024) { importFile.fail('The input file exceeds 16 MiB.'); return; }
        switch (files[0].name.split(".").pop().toLowerCase()) {
            ...
            case "xlsx":
                ...
                break;
            default: importFile.fail('Supported input formats: CSV, TSV, TXT, XLS and XLSX.');
        }
    },
    fail: function (message) {
        jsonInput = [];
        document.getElementById('import-help').textContent = 'Input error';
        document.getElementById('samples').textContent = message;
        $('#batch').button().attr('disabled', true).addClass('ui-state-disabled');
    },
    load: function (value) {
        for (let i = 0; i < jsonInput.length; i++) {
            if (jsonInput[i].description === value) {
                ...
                document.getElementById("sample-" + speciesNames[species]).innerHTML = "Sample <span class='text-purple'>" + escapeHtml(value) + "</span> loaded";
    ...
    _callback: function (results) {
        ...
        if (status > 0) {
            if (jsonInput.length === 1) {
                ...
                for (let i = 0; i < jsonInput.length; i++) {
                    samples += "<a class='sample' onclick='importFile.load(this.innerText)'>" + escapeHtml(jsonInput[i].description) + "</a><br>"
                }
                samples += "<br>";
    ...
    _clean: function (json) {
        ...
        for (let i = 0; i < json.length; i++) {
            if (json[i]["SampleReferenceNbr"] !== undefined) {
                ...
                json[i].description = importFile._renameCheck(names, json[i]["Sample"]);
    ...
    _validate: function(json) {
        ...
        for (let i = 0; i < json.length; i++) {
            let found = 0;
            for (let property of Object.keys(json[i])) {
                if (json[i].hasOwnProperty(property) && property !== "description") {
                    let name = this._format(property);
                    if (name !== property) {
                        json[i][name] = String(json[i][property]);
                        delete json[i][property]
                    }
                    if (markers[species]["default"].includes(name) || markers[species]["optional"].includes(name)) {
                        const value = String(json[i][name]).replace(/\s/g, '');
                        if (!value) continue;
                        const pattern = name === 'Amelogenin' ? /^[XY](,[XY])?$/i : /^\d{1,3}(\.\d)?(,\d{1,3}(\.\d)?)*$/;
                        if (!pattern.test(value)) throw new Error('Invalid alleles in sample ' + json[i].description + ', marker ' + name);
                        json[i][name] = value; found++; c++;
                    }
                }
            }
            if (!found) throw new Error('No compatible marker in sample ' + json[i].description);
        }
        return c;
    },
    _csv: function (file) {
        ...
        Papa.parse(file, {
            ...
            error: function () { if (ticket === queryEpoch) importFile.fail('Could not read the input file.'); },
            complete: function (results) {
                if (ticket !== queryEpoch) return;
                if (results.errors && results.errors.length) { importFile.fail('Malformed delimited file: ' + results.errors[0].message); return; }
                try { importFile._callback(results.data); } catch (e) { importFile.fail('Invalid sample data: ' + e.message); }
            }
        });
    },
    _spreadsheet: function (file, library) {
        const ticket = queryEpoch;
        const reader = activeReader = new FileReader();
        reader.onerror = () => { if (ticket === queryEpoch) importFile.fail('Could not read the input file.'); };
        reader.onload = event => {
            if (ticket !== queryEpoch) return;
            try {
                const workbook = library.read(event.target.result, {type: 'binary'});
                if (!workbook.SheetNames.length) throw new Error('No worksheet');
                const rows = library.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], {raw: false, defval: ''});
                importFile._callback(rows);
            } catch (e) { importFile.fail('Invalid spreadsheet: ' + e.message); }
        };
        reader.readAsBinaryString(file);
    },
    _xls: function (file) { importFile._spreadsheet(file, XLS); },
    _xlsx: function (file) { importFile._spreadsheet(file, XLSX); },
    openDialog: function () {
...
let exportTable = {
    snapshot: function (format) {
        // Preserve the full grouped scientific result and identify every displayed profile row.
        const result = JSON.parse(JSON.stringify(jsonResponse));
        result.rowOrder = Array.from(document.querySelectorAll('#table-results tr[data-result]'),
            row => [Number(row.dataset.result), Number(row.dataset.profile)]);
        if (format) result.outputFormat = format;
        else delete result.outputFormat;
        return result;
    },
    toXlsx: function (name) {
        const output = exportTable.snapshot("xlsx");
        const ticket = queryEpoch;
        const request = $.ajax({
            type: "POST",
        ...
        activeExports.add(request);request.always(() => activeExports.delete(request));
    },
    toCsv: function (name) {
        const output = exportTable.snapshot("csv");
        const ticket = queryEpoch;
        const request = $.ajax({
            type: "POST",
            url: "/str-search/api/conversion",
    ...
    toJson: function (name) {
        const output = exportTable.snapshot();

        let blob = new Blob([JSON.stringify(output, undefined, 2)], {type: "text/plain"});
    ...
    dialogImport = $("#dialog-import").dialog({
        ...
        buttons: [{
                ...
                click: function() {
                    cancelPending();
                    const ticket = queryEpoch;
                    const importExtension = $('#import-extension').val();
                    const filename = $('#import-name').val() + '.' + (importExtension === 'csv' ? 'zip' : importExtension);
                    html.addClass("waiting");
