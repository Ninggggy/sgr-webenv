...
With the data archive extracted under this environment's `data/` directory (author/test use only), run `python3 tests/data_sources.py`, `python3 tests/index_sources.py` and `python3 tests/search_sources.py`. These reconcile the SQLite representation against the independent XML source and archived official search responses, not expected answers generated by the running application. Equal-name ties and the incomplete original large-result page are reported separately. `validation/` outputs are ignored by Git and never mounted by runtime Compose.
