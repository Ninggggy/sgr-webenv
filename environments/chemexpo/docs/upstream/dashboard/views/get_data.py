...
def get_data(
    ...
    template_name=select_template_for_app(
        internal="get_data/get_data_internal.html", external="get_data/get_data.html"
    ),
):
...
def download_chem_stats(stats):
    ...
    filename = get_download_filename("chem_summary_metrics", "csv")
...
def download_PUCTags(request):
    ...
    response = HttpResponse(content_type="text/csv")
    filename = get_download_filename("PUC_Attributes", "csv")
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
    writer = csv.writer(response)
    cols = ["PUC Attribute", "Definition"]
...
def download_functional_uses(request):
    ...
    filename = get_download_filename("bulk_functional_uses", "csv")
