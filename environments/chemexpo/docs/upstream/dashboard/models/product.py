...
class Product(CommonInfo):
    ...
    tags_name = models.TextField(
        blank=True, help_text="semi column separated tag names"
    )
    ...
    item_id = models.CharField(
        max_length=25, null=True, blank=True, help_text="item ID"
    )
    parent_item_id = models.CharField(
        max_length=25, null=True, blank=True, help_text="parent item ID"
    )
    ...
    def uber_puc(self):
        """
        Returns the uberpuc as derived at runtime from
        the classification_method.rank order. This method
        does not use the product_uber_puc view or the
        is_uber_puc attribute.
        """
    ...
    def get_puc_tag_list(self):
        ...
        return ", ".join(o.name for o in all_uber_tags)
