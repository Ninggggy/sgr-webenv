...
class DSSToxLookup(CommonInfo):
    ...
    true_cas = models.CharField("Curated CAS", max_length=50, blank=True)
    true_chemname = models.CharField(
        "Curated chemical name", max_length=500, blank=True
    )
    ...
    def get_tag_sets(self):
        ...
        for kw_set in counter:
            ...
            if one[kw_hash]:
                exact_count = 0
                for i in two[kw_hash]:
                    if i in docs_cache:
                        doc = docs_cache[i]
                    else:
                        doc = apps.get_model(
                            "dashboard", "ExtractedCPCat"
                        ).objects.filter(pk=i)
                        docs_cache[i] = doc
                    if len(doc[0].get_tagset()) == len(kw_set):
                        all_exact_doc_ids.add(doc[0].pk)

        all_docs_qs = DataDocument.objects.filter(
            Q(extractedtext__rawchem__dsstox__sid=self.sid)
        ).distinct()
        for doc in all_docs_qs:
            all_docs.add(doc.pk)

        orphan_doc_ids = all_docs - all_exact_doc_ids
        self.final_docs[self.pk] = []

        for kw_set in counter:
            kw_hash = hash(kw_set)
            if one[kw_hash]:
                exact_count = 0
                self.final_docs[self.pk].append([])
                for i in two[kw_hash]:
                    doc = docs_cache[i]
                    if (
                        len(doc[0].get_tagset()) == len(kw_set)
                        or doc[0].pk in orphan_doc_ids
                    ):
                        exact_count += 1
                        self.final_docs[self.pk][-1].append(doc[0].pk)
                keysets.append(
                    KeywordSet(
                        keywords=kw_set, count=exact_count, presence_id=one[kw_hash]
                    )
                )
        return keysets
