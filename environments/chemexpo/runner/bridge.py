...
SOCKET="/tmp/chemexpo-browser.sock";ORIGIN="http://chemexpo-web:8080";OUTPUT=Path("/tmp/output")
def allowed(url):
 u=urlsplit(url)
 return u.scheme=="http" and u.hostname=="chemexpo-web" and u.port==8080 and not u.username and not u.password
   ...
   with conn:
    ...
    try:
     ...
     if action=="goto":
      url=req["url"]
      parsed=urlsplit(url)
      if parsed.scheme=="https" and parsed.hostname=="comptox.epa.gov" and parsed.port in (None,443) and not parsed.username and not parsed.password and parsed.path.startswith("/chemexpo/"):
       url=ORIGIN+parsed.path+("?"+parsed.query if parsed.query else "")
      elif url.startswith("/"):url=ORIGIN+url
      if not allowed(url):raise ValueError("Only the local ChemExpo website is allowed")
