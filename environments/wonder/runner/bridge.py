...
SOCKET="/tmp/wonder-browser.sock";ORIGIN="http://wonder-web:8080";OUTPUT=Path("/tmp/output")
def allowed(url):
 u=urlsplit(url)
 return u.scheme=="http" and u.hostname=="wonder-web" and u.port==8080 and not u.username and not u.password
   ...
   with conn:
    ...
    try:
     ...
     if action=="goto":
      ...
      if not allowed(url):raise ValueError("Only the local WONDER website is allowed")
      page.goto(url,wait_until="networkidle")
     elif action=="select":page.locator(selector).select_option(req["value"] if isinstance(req["value"],list) else str(req["value"]))
      ...
      item=downloads[req["id"]];path=Path(item["path"]);offset=int(req.get("offset",0));limit=int(req.get("max_bytes",1000000))
      if offset<0 or not 1<=limit<=1000000:raise ValueError("Invalid download byte range")
      with path.open("rb") as source:source.seek(offset);b=source.read(limit)
      size=path.stat().st_size
      conn.sendall((json.dumps({"name":item["name"],"text":b.decode("utf-8",errors="replace"),"base64":base64.b64encode(b).decode(),"offset":offset,"next_offset":offset+len(b) if offset+len(b)<size else None,"total_bytes":size})+"\n").encode());continue
