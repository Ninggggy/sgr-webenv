...
a direct read-only check confirmed the excluded chart library remains retrievable through an old GitHub commit. The current private repository must not be made public until those objects are removed or publication moves to a clean independent repository. See `PUBLICATION.md`.
