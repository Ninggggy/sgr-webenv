...
SGR-WebEnv 将网站应用、固定数据和受限浏览器打包为 Docker 环境，支持在本地开展交互式搜索、页面浏览和数据查询研究，无需访问作者的部署服务器。资源准备完成后，已支持的工作流可以离线运行，无需连接原网站。

仓库包含六个网站的独立复现。各环境均有明确的数据与功能范围；材料可安装不代表与原网站完全等价。本项目与原网站及其运营机构无隶属或背书关系。
...
|---|---|---|---:|
| NOAA Climate at a Glance | 部分发布 | 提供源码与气候数据；未包含专有图表组件，暂不支持完整安装。Rank 与并列结果仍有差异。 | 8080* |
| Census / ACS | 可安装 · 0.2.1 | 已归档的地区、年份与表格；不覆盖全国所有 ACS 产品。 | 8081 |
| CDC WONDER | 可安装 · 0.3 | 文档范围内的全国分组查询；存在保护规则差异，数据附 NCHS 使用条件。 | 8082 |
...
*8080 为 NOAA 部署配置中的预留端口；当前部分发布材料无法直接安装为完整环境。*
